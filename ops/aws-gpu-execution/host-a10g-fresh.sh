#!/bin/bash
# Hash-verified public fresh-proof bundle is staged by the operator before invocation.
set -euo pipefail
bundle=$1
results=$2
deadline=$3
[[ "$bundle" = /opt/qsb-a10g-fresh && "$results" = /var/tmp/qsb-a10g-fresh-results ]]
[[ "$deadline" =~ ^[0-9]{10}$ ]]
...
[ "$((deadline-$(date -u +%s)))" -ge 1020 ]
nvidia-smi --query-gpu=name,uuid,driver_version --format=csv,noheader > "$results/gpu.txt"
set +e
timeout --signal=TERM --kill-after=10s 960 docker run --name qsb-a10g-fresh --rm --gpus all \
  --network none --read-only --cap-drop ALL --security-opt no-new-privileges \
  --tmpfs /tmp:rw,nosuid,nodev,size=256m \
  --mount "type=bind,src=$bundle,dst=/gate,readonly" \
  --mount "type=bind,src=$results,dst=/results" \
  --entrypoint python3 "$image" /gate/run_fresh_sm86.py /gate/batch.json /results/result.json > "$results/gate.log" 2>&1
status=$?
set -e
# Killing the Docker client does not reliably stop its container.
docker rm --force qsb-a10g-fresh >/dev/null 2>&1 || true

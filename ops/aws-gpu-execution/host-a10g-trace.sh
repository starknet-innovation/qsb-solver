...
receipt_sha=$4
fixture_sha=$5
[[ "$receipt_sha" =~ ^[0-9a-f]{64}$ && "$fixture_sha" =~ ^[0-9a-f]{64}$ ]]
...
timeout --signal=TERM --kill-after=10s 720 docker run --name qsb-a10g-gate --rm --gpus all \
  ...
  --entrypoint python3 "$image" /gate/run_a10g_trace.py /gate /results/result.json "$receipt_sha" "$fixture_sha" > "$results/gate.log" 2>&1
status=$?
set -e

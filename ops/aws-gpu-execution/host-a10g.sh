...
timeout --signal=TERM --kill-after=10s 720 docker run --name qsb-a10g-gate --rm --gpus all \
  ...
  --entrypoint python3 "$image" /gate/run_a10g.py /results/result.json > "$results/gate.log" 2>&1
status=$?
set -e
...
print('QSB_PUBLIC_RESULT='+base64.b64encode(gzip.compress(json.dumps(files).encode())).decode())
PY
exit "$status"
